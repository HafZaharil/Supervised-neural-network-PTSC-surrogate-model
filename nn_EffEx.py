...
# -----------------------------
# 11. Export test predictions
# -----------------------------
pd.DataFrame({
    "Actual": y_test.flatten(),
    "Predicted": y_pred_test.flatten()
}).to_csv(
    "outputs/effex/nn_test_predictions_effex.csv",
    index=False
)
...
pd.DataFrame({
    ...
    "outputs/effex/nn_validation_predictions_effex.csv",
    index=False
)

# -----------------------------
# 13. Summary metrics CSV
# -----------------------------
...
summary_metrics.to_csv(
    "outputs/effex/summary_metrics_effex.csv",
    index=False
)
...
def find_best_for_ambient(model, scaler, dni, tamb_c, k_factor):
    ...
    best_coarse = df_coarse.loc[df_coarse["Predicted_EffEX"].idxmax()]
    ...
    best = df_fine.loc[df_fine["Predicted_EffEX"].idxmax()].copy()
        ...
        rows.append(
            {
                ...
                "Tamb": float(tamb_k),
                "K": float(k_factor),
                "Predicted_EffEX": float(y_pred[best_idx]),
            }
        )
...
def run_surrogate_max_finder(model, scaler):
    print("\n============================================================")
    print("SURROGATE MAX FINDER — Neural Network (EffEX)")
    ...
    while True:
        ...
        print("\n===== BEST (MAX PREDICTED EFFEX) =====")
        print(f"Predicted_EffEX: {best['Predicted_EffEX']:.3f}")
