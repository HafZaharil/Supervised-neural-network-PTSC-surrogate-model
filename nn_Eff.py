...
pd.DataFrame({
    ...
    "outputs/eff/nn_test_predictions_eff.csv",
    index=False
)
...
pd.DataFrame({
    ...
    "outputs/eff/nn_validation_predictions_eff.csv",
    index=False
)
...
summary_metrics.to_csv(
    "outputs/eff/summary_metrics_eff.csv",
    index=False
)
...
def run_surrogate_max_finder(model, scaler):
    ...
    while True:
        ...
        print(f"Predicted_Eff: {best['Predicted_Eff']:.3f}")
